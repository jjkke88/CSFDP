...
class DataProcess(object):
    def __init__(self, file_name, delimiter, d_c, rho_ratio=0.2, sigma_ratio=0.2):
        ...
        self.plt_show()
        self.rho_min = self.rho_vector_sort[-1]*rho_ratio
        self.sigma_min = self.sigma_vector_sort[-1]*sigma_ratio
        self.get_center()
    ...
    def get_data_from_file(self):
        ...
        distance_matrix = np.zeros((self.sample_number, self.sample_number), dtype=float)
        ...
        distance_vector = np.concatenate(distance_matrix)
        distance_vector_sort = np.sort(distance_vector)
        index = int(pms.d_c_ratio*distance_vector.shape[0])
        self.d_c = distance_vector_sort[index]
        ...
        self.rho_vector_sort = np.sort(self.rho_vector)
        print "max_neighbour_number:" + str(np.max(self.rho_vector))

    def calculate_sigma(self):
        sigma_vector = np.zeros(self.sample_number)
        for i in xrange(self.sample_number):
        ...
        self.sigma_vector_sort = np.sort(self.sigma_vector)
    ...
    def get_center(self):
        ...
        for i in xrange(self.sample_number):
            if self.rho_vector[i]>=self.rho_min and self.sigma_vector[i]>=self.sigma_min:
                center.append(i)
        self.center = center
        print "center number:%d"%(len(center))

    def classify_others(self):
        result = np.zeros(self.sample_number)
        for center_no in xrange(len(self.center)):
...
if __name__ == "__main__":
    ...
    import time
    start = time.time()
    data_process = DataProcess("dist_data.dat", '\t', 0, rho_ratio=0.8, sigma_ratio=0.4)
    end = time.time()
    print "process time:"+str(end-start)
    print data_process.result
    raw_data = np.genfromtxt("raw_data.dat", delimiter=" ", names=['x' , 'y'] ,
                             dtype="f8,f8")
